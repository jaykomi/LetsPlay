...
int green_box_state; // 0 = niks, 1 = wrong, 2 = victory, 3 = reset
...
void loop() {
  ...
  bool buttonState = digitalRead(button);
   ...
   while ( network.available() ) {     // Is there any incoming data?
    ...
    network.read(header2, &green_box_state, sizeof(green_box_state)); // Read the incoming data
    Serial.println(green_box_state); // Read the incoming data)
     digitalWrite(PIN_LED, HIGH);  //cal reset
   }

  if (buttonState == 1 && green_box_state != 30) {
